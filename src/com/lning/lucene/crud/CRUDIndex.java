...
public class CRUDIndex {
	...
	public void updateLucene() throws Exception{
		...
		doc.add(new TextField("title", "solr入门级教程", Store.YES));
		doc.add(new TextField("content", "solr 是一个基于 Java 的全文信息检索工具包，它不是一个完整的搜索应用程序", Store.YES));
	...
	@Test
	public void deleteLucene() throws Exception{
		//1.创建目录类，path是索引库的位置
		String path="F:\\indexs";
		FSDirectory directory = FSDirectory.open(new File(path));
		//2.创建分词器
		...
		//4.创建索引
		IndexWriter indexWriter=new IndexWriter(directory, writerConfig);
		indexWriter.deleteDocuments(new Term("id","10010"));
		//需要提交索引
		indexWriter.commit();
	}
